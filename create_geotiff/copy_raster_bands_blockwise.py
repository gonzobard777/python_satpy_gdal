...
def copy_raster_bands_blockwise(
        src: gdal.Dataset,
        dst: gdal.Dataset,
        preferred_x: int = 2048,
        preferred_y: int = 2048,
        min_block_y: int = 8,
) -> None:
    """
    ...
    Стратегия:
      ...
      - Если это "scanline" (PNG/JPEG часто дают block_y=1) — используем preferred_x/preferred_y
    ...
    chunk_x, chunk_y = _choose_chunk_size(width=width,
                                          height=height,
                                          block_x=block_x,
                                          block_y=block_y,
                                          preferred_x=preferred_x,
                                          preferred_y=preferred_y,
                                          min_block_y=min_block_y)
...
def _choose_chunk_size(
    ...
    if block_y <= min_block_y:
        chunk_x, chunk_y = preferred_x, preferred_y
    else:
